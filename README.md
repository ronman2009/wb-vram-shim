...
**两款游戏实测验证**（GTA V Enhanced：+1.2 GiB；Forza Horizon 5：峰值 6.65 → 7.51 GiB），**v3.2 起默认对所有游戏生效**。
...
| 游戏 | 指标 | 无补丁 | 装 wb-vram-shim |
|---|---|---|---|
| GTA V Enhanced (DX12) | 实际可占用 | **≈ 5.0 GiB** | **≈ 6.2 GiB（+1.2 GiB）** |
| | 看到的额度 | 5861 MiB（动态缩水） | 恒定、实时计算的值 |
| Forza Horizon 5 (DX12) | 峰值占用 | ≈ 6.65 GiB | **≈ 7.51 GiB（泛化实测 ✔）** |

显存总量上报同时从 8160 MiB 修正为 8192 MiB（物理完整值）。
...
| `WBVRAM_APPS` | 空 | 逗号分隔 appid = **只**对这些游戏生效（默认空 = 全部游戏） |
...
- ✅ **默认对所有游戏生效**（v3.2 起解除 appid 白名单）；泛化已在两款 DX12 游戏上实测，
  辅助进程（xalia / webview2 等）自动跳过
...
- **宿主端测试台 14 场景**（假 DXVK + 假 NVML + 假 `/proc`，不需要 GPU），含
