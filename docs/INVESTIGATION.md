...
## 7. 泛化测试：Forza Horizon 5（2026-09-25 晚）

**目的**：验证 v3.1/v3.2 的机制是否只对 GTA 成立，以及 GE-Proton 是否已自带修复。

**条件**：FH5（appid 1551360）本就在用 GE-Proton11-7（前缀软链 + dxgi 指纹确认）。
A 组（无补丁）与 B 组（v3.1+）先后实测；A 组期间白名单恰未含 1551360，
日志证实所有 FH5 进程"只做转发"——对照组天然干净。

| | A 组（无补丁） | B 组（v3.1+） |
|---|---|---|
| 峰值显存占用 | ≈ 6.65 GiB | **≈ 7.51 GiB** |

**结论**：
1. **泛化成立** —— 同一个 dxgi.dll、同一套"NVML 逐进程枚举 + cmdline 认自己"，
   在第二款 DX12 游戏上直接生效，游戏峰值占用 +0.86 GiB。
2. **GE-Proton 未包含修复** —— 无补丁时 FH5 也存在同样的预算压制（6.65 vs 7.51），
   且 DXVK master 源码的预算公式与调研时完全一致，无上游修复迹象。
3. 白名单安全阀在跨游戏场景按设计工作（xalia 拿不到 appid → 不改写；
   webview2 → 转发）。

**v3.2**：鉴于泛化成立，解除 appid 白名单——默认对所有游戏生效；
`WBVRAM_APPS` 可限定；拿不到 appid 的辅助进程仍不改写（保守线保留）。
